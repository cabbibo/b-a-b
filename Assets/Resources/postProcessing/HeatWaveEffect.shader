Shader "PostProcessing/HeatWaveEffect"
{
    ...
    float _HeatWaveStart;
    float _HeatWaveEnd;
    float _HeatWaveAberration;
...
*/
    ...
    float4 Frag( VaryingsDefault v ) : SV_Target
    {
        ...
        // float value = pow( saturate( ( distance - _HeatWaveStart ) / ( _HeatWaveEnd - _HeatWaveStart ) ) , 1 );
  ...
  */
        ...
        float2 newUV = uvR - float2( 0 , _Time.y * _WaveSpeed );

        float3 eps = float3( .001 , 0 , 0 );

        float3 nP1 = float3( newUV.x , newUV.y , 0 ) * _WaveSize;

        float2 noiseGrad = float2(
            noise( nP1 + eps.xyy ) - noise( nP1 - eps.xyy ) ,
            noise( nP1 + eps.yxy ) - noise( nP1 - eps.yxy )
        );

        float value = pow( saturate( ( distance - _HeatWaveStart ) / ( _HeatWaveEnd - _HeatWaveStart ) ) , 1 );

        uvR += noiseGrad * _WaveIntensity * value * ( 1 + _HeatWaveAberration * 0 );
        uvG += noiseGrad * _WaveIntensity * value * ( 1 + _HeatWaveAberration * 1 );
        uvB += noiseGrad * _WaveIntensity * value * ( 1 + _HeatWaveAberration * 2 );

        color.x = tex2D( _MainTex , uvR ).x;
        color.y = tex2D( _MainTex , uvG ).y;
        color.z = tex2D( _MainTex , uvB ).z;
