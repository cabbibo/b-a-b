...
float _LoopFaders[16];

float GetFaderValue(int id){
    return _LoopFaders[id];
}

float GetFaderValue(float id ){

    int fID = floor( id * (_NumStems));
    return _LoopFaders[fID];
}
