...
float3 terrainWorldPos( float4 pos ){
    ...
    float4 c = tex2Dlod(_HeightMap , float4(wp.xz / _MapSize.xz,0,0) );
    wp.xyz += float3(0,1,0) * c.r * _MapSize.y;
    return wp;
}
...
float3 terrainWorldPos( float3 pos ){
    float4 c = tex2Dlod(_HeightMap , float4(pos.xz / _MapSize.xz,0,0) );
   float3 p = float3(pos.x,0,pos.z) + float3(0,1,0) * c.r *_MapSize.y;
    return p;
}
...
float4 terrainSampleColor( float4 pos ){
  float3 wp = mul( unity_ObjectToWorld, pos ).xyz;
  return tex2Dlod(_HeightMap , float4(wp.xz  / _MapSize.xz,0,0) );
}
