// Upgrade NOTE: replaced '_Object2World' with 'unity_ObjectToWorld'

Shader "Debug/ForcePretty1" {
    Properties {

    _Color ("Color", Color) = (1,1,1,1)
    _Size ("Size", float) = .01
    _Forwards ("Forwards", float) = 1
    _MainTex ("Texture", 2D) = "white" {}
    _MaxPower ("MaxPower", float) = 1
    _OverallMultiplier ("OverallMultiplier", float) = 1
    }


  SubShader{
          Tags { "Queue"="Overlay+1000" "IgnoreProjector"="True" "RenderType"="Transparent+100000" }
      Blend One One
    //	AlphaTest Greater .01
      //ColorMask RGB
      Cull Off 
      ZWrite Off 
    Pass{

      CGPROGRAM
      
      #pragma target 4.5

      #pragma vertex vert
      #pragma fragment frag

      #include "UnityCG.cginc"
      #include "Assets/Resources/Shaders/Chunks/hsv.cginc"

          
    struct Force{
      float3 force;
      float3 pos;
    };






      uniform int _Count;
      uniform float _Size;
      uniform float _Forwards;
      uniform float3 _Color;
      uniform float _MaxPower;

      
      StructuredBuffer<Force> _ForceBuffer;


      //uniform float4x4 worldMat;

      //A simple input struct for our pixel shader step containing a position.
      struct varyings {
          float4 pos      : SV_POSITION;
          float3 nor      : TEXCOORD0;
          float3 worldPos : TEXCOORD1;
          float3 eye      : TEXCOORD2;
          float2 uv       : TEXCOORD4;
          float id        : TEXCOORD5;
      };


uniform float4x4 _Transform;
//Our vertex function simply fetches a point from the buffer corresponding to the vertex index
//which we transform with the view-projection matrix before passing to the pixel program.
varyings vert (uint id : SV_VertexID){

  varyings o;

  int base = id / 6;
  int alternate = id %6;

  if( base < _Count ){

      float3 extra = float3(0,0,0);

    //float3 l = UNITY_MATRIX_V[0].xyz;
    //float3 u = UNITY_MATRIX_V[1].xyz;
    
    float2 uv = float2(0,0);

    Force v = _ForceBuffer[base];


  float3 d = v.force;//normalize(v.force);
    if( length(d) > _MaxPower ){
      d = normalize(d) * _MaxPower;
    }
    

      float3 dN = normalize(d);

      float3 u = dN * length(d)  *_Size;
      float3 l = normalize(cross(d,UNITY_MATRIX_V[2].xyz)) * length(d) * _Size;


    float3 p1 = -l;
    float3 p2 = l;
    float3 p3 = l+u*2;
    
    float3 p4 = -l;
    float3 p5 = l+u*2;
    float3 p6 = -l+u*2;

    if( alternate == 0 ){ extra = p1; uv = float2(0,0); }
    if( alternate == 1 ){ extra = p2; uv = float2(1,0); }
    if( alternate == 2 ){ extra = p3; uv = float2(1,1); }

    
    if( alternate == 3 ){ extra = p4; uv = float2(0,0); }
    if( alternate == 4 ){ extra = p5; uv = float2(1,1); }
    if( alternate == 5 ){ extra = p6; uv = float2(0,1); }

    

      o.worldPos =v.pos+ extra;// mul(_Transform, float4((v.pos) ,1));
      ///o.worldPos +=  extra * _Size;

      o.eye = _WorldSpaceCameraPos - o.worldPos;
      o.nor =d;//v.nor;
      o.uv = uv;
      o.id = base;
    
      o.pos = mul (UNITY_MATRIX_VP, float4(o.worldPos,1.0f));

  }

  return o;

}


      sampler2D _MainTex;

      float _OverallMultiplier;

//Pixel function returns a solid color for each point.
float4 frag (varyings v) : COLOR {


  float3 col = 1;

  float2 uv = v.uv;

  col.x = uv.x;
  col.y = uv.y;

  float4 tex = tex2D(_MainTex, uv);

  col = tex;

  

  if( length(tex.rgb)> 1){
    discard;
  }

  col = normalize(v.nor) * .5 + .5;

col *= length(v.nor) * _OverallMultiplier * _Color;


    return float4(col,1 ) ;
}

      ENDCG

    }
  }

  Fallback Off


}
