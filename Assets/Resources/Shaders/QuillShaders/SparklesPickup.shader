...
Shader"World/SparklesPickup"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            varyings vert( appdata_full2 vert )
            {
                ...
                UNITY_TRANSFER_INSTANCE_ID( vert , o );
    ...
    instanceID = vert.instanceID;
            ...
            float4 frag( varyings v ) : COLOR
            {
                ...
                //  shadowCol *= abs( v.uv.x - .5 );

                col = shadowStep * col * float3( 1 , 1 , .6 ) * ( shadowCol * 1 + .9 ) * triplanar +
                    pow( ( 1 - m ) , 1 ) * 2 * clamp( ( 1 - shadowStep ) * shadowCol * .4 * col * 20 , 0 , 1 ) + .05 *
                    float3( .4 , 0.3 , 1 ) * ( 1 - shadowStep ) * floor( pow( 1 - m , 4 ) * 5 ) / 5;;
                // float3(.1,.2,.5);
