...
Shader "Unlit/collectableRing"
{
    Properties
    {
        ...
        _Inside("Inside",float) = 0
        _CollectedAmount("CollectedAmount",float) = 0
        _Finished("Finished",float) = 0
...
float _Inside;
float _CollectedAmount;
float _Finished;

float _CollectablePoints;
            v2f vert (appdata v)
            {
                v2f o;

                o.world = mul(unity_ObjectToWorld , v.vertex );
                float3 fPos = terrainPos( o.world );

                o.nor = terrainNor( o.world);
            ...
            float n = noise( v.world * 2 );
                float r = length(v.uv - .5) * 2;
                float a = atan2( v.uv.x- .5 , v.uv.y- .5);
                
                float d = length(v.uv - .5) * 2;
                if( d > 1-n*.1){ discard;}
                /*if( d < .9+n*.1 - _Inside * .9 ){
                    discard;
                }*/
                if( sin(d * 50 ) + n   > _Finished + _Inside ){
                    discard;
                }

                if( d < .9 - n*.1- _CollectedAmount / _CollectablePoints){
                    discard;
                }
            
                float speed = -_Time.y * ( _CollectedAmount * 3 + 1 )  * (_Inside+.1);

                //if( sin( d * 40 + _Time.y ) < .5 + d * .4 ){ discard;}
                // sample the texture
                float3 col = _Color*(sin( a * 10 + n + speed  + floor(d * 50/3.14159) )+1)/2;// v.nor * .5 + .5; 
                return float4(col, 1);
            }
            ENDCG
        }
