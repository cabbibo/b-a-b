...
varyings vert (uint id : SV_VertexID){
  ...
  int featherID = base/4;
  int inFeatherID = base % 4;
...
Feather v = _FeatherBuffer[featherID];
...
float3 p1 = v.pos + ( - l - u) * _Size;
float3 p2 = v.pos + ( + l - u) * _Size;
float3 p3 = v.pos + ( - l + u) * _Size;
float3 p4 = v.pos + ( + l + u) * _Size;


  u = mul( v.ltw , float4(0,0,1,0)).xyz;
  l = mul( v.ltw , float4(1,0,0,0)).xyz;
...
}else if( inFeatherID == 2 ){
  float3 u2 = mul( v.ltw , float4(0,1,0,0)).xyz;
  float3 l2 = mul( v.ltw , float4(1,0,0,0)).xyz;

  float3 basePos = v.pos - u * 46 * _Size;
  u2 *= 4;
  l2 *= .3;
  p1 = basePos + ( - l2 + u2 ) * _Size;
  p2 = basePos + ( + l2 + u2 ) * _Size;
  p3 = basePos + ( - l2 - u2 ) * _Size;
  p4 = basePos + ( + l2 - u2  ) * _Size;    


}else if( inFeatherID == 3 ){
  float3 u2 = mul( v.ltw , float4(1,0,0,0)).xyz;
  float3 l2 = mul( v.ltw , float4(0,1,0,0)).xyz;
  float3 basePos = v.pos - u * 46 * _Size;
...
float4 frag (varyings v) : COLOR {
    ...
    float3 col = hsv(v.featherType * .2 + v.inFeatherID * .1,1,1);
