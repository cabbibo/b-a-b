...
varyings vert (uint id : SV_VertexID){
  ...
  if( base < _Count ){
    ...
    if( alternate == 0 ){ extra = v.pos + (- u - l) * _Size ; uv = float2(0,0); }
    if( alternate == 1 ){ extra = v.pos + (- u + l) * _Size ; uv = float2(1,0); }
    if( alternate == 2 ){ extra = v.pos + (+ u + l) * _Size ; uv = float2(1,1); }
    if( alternate == 3 ){ extra = v.pos + (- u - l) * _Size ; uv = float2(0,0); }
