Shader "IMMAT/Wren/tutorialCurveGrass"
{
    Properties {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f { 
                ...
                half3 bi : TEXCOORD15; // tangent.z, bitangent.z, normal.z
                // in v2f struct;
                LIGHTING_COORDS(5,6)
            ...
            fixed4 frag (v2f v) : SV_Target
            {
                ...
                float3 cMap = tex2D( _FullColorMap , float2( v.uv.x , .6));


                float4 s3 = tex2D( _TextureMap , v.uv );
                float4 s2 = tex2D( _ColorMap , float2( m/3, 0) );
                float3 shiny = tex2D(_ShinyMap,v.uv);

                float3 fCol=s2;//lerp( pow(length(tCol),3)* .3* s2*3, pow(length(tCol),3) * .1 * s2 ,shiny.x)  *_Saturation + _Brightness;//*shiny.x * fLCol;//fLCol*s3* skyColor;//v.nor * .5 + .5;
