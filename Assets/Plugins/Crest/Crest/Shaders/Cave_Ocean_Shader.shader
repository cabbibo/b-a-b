    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 PainterlyColor( float3 color1 , float3 color2 , float m , float2 uv )
            {
                ...
                float4 p = tex2D( _PainterlyMap , uv );
                ...
                fLCol.xyz = lerp( color1 , color2 , 1 - pow( fLCol , 4 ) );
            ...
            half4 Frag( const Varyings input , const bool i_isFrontFace : SV_IsFrontFace ) : SV_Target
            {
                ...
                */
                ...
                float3 dummy   = 0.;
                float3 n_pixel = float3( 0.0 , .3 , 0.0 );
                half   sss     = 0.;
                #if _FOAM_ON
                float foam = 0.0;
                #endif
               ...
               col );
                ...
                float eyeMatch = dot( normalize( eye ) , normal );
                ...
                */
                ...
                //col = floor( lightMatch * lightMatch * 1 + .5 ) / 2 * _LightColor0.rgb;

                //col = lightMatch;
                if ( dot( flatNormal , normalize( eye ) ) < 0.0 )
                {
                    // col += float3( 1 , 1 , 1 );
                }
                ...
                col = dot( normalize( eye ) , normal ); // * .5 + .5;
