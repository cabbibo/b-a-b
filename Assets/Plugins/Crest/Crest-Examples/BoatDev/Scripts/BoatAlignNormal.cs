...
namespace Crest.Examples
{
    ...
    /// </summary>
    [AddComponentMenu( Internal.Constants.MENU_PREFIX_EXAMPLE + "Boat Align Normal" )]
    [ExecuteAlways]
...
#pragma warning restore 414
        ...
        [Tooltip(
            "Computes a separate normal based on boat length to get more accurate orientations, at the cost of an extra collision sample." )]
        public bool _useBoatLength = false;

        [Tooltip( "Length dimension of boat. Only used if Use Boat Length is enabled." )]
        [Predicated( "_useBoatLength" )]
        [DecoratedField]
        ...
        private SampleHeightHelper _sampleHeightHelperLengthwise = new();
        ...
        private void FixedUpdate()
        {
            ...
            if ( _accelerateDownhill > 0f ) {
                _rb.AddForce( new Vector3( normal.x , 0f , normal.z ) * -Physics.gravity.y * _accelerateDownhill ,
                    ForceMode.Acceleration );
            }

            // apply drag relative to water
            var forcePosition = _rb.position + _forceHeightOffset * Vector3.up;
            _rb.AddForceAtPosition( Vector3.up * Vector3.Dot( Vector3.up , -velocityRelativeToWater ) * _dragInWaterUp , forcePosition ,
                ForceMode.Acceleration );
            _rb.AddForceAtPosition( transform.right * Vector3.Dot( transform.right , -velocityRelativeToWater ) * _dragInWaterRight ,
                forcePosition , ForceMode.Acceleration );
            _rb.AddForceAtPosition(
                transform.forward * Vector3.Dot( transform.forward , -velocityRelativeToWater ) * _dragInWaterForward , forcePosition ,
                ForceMode.Acceleration );

            float forward = _throttleBias;
#if INPUT_SYSTEM_ENABLED
            float rawForward = !Application.isFocused
                ? 0
