...
normalize( mul(
    unity_ObjectToWorld , float4( n.xyz , 0.0f ) ) ) ,
_IndexOfRefraction );
            ...
            {
                return float3( tri( p.y + tri( p.z ) ) , tri( p.z + tri( p.x ) ) , tri( p.y + tri( p.x ) ) );
            }
            ...
            float4 frag( varyings v ) : COLOR
            {
                ...
                for ( int i = 0; i < _NumSteps; i++ )
                {
                    ...
                    float centerOrbDensity = ( ( _CenterOrbImportance ) / ( pow(
                        length( p - _CenterOrbOffset ) , _CenterOrbFalloffSharpness ) * _CenterOrbFalloff ) );
                    ...
                    col = .99 * col + _Color * ( dot( float3( 0 , 1 , 0 ) , nor ) + 1 );
                    // (nor * .5+.5);//lerp( lerp(_BaseColor,_CenterOrbColor * (nor * .5 + .5) , saturate(centerOrbDensity)),_NoiseColor* (nor * .5 + .5) , saturate(noiseDensity ));// saturate(dot(v.lightDir , nor)) * .1 *c;//hsv(c,.4, dT3D(p*3,float3(0,-1,0))) * c;//hsv(c * .8 + .3,1,1)*c;;// hsv(smoke,1,1) * saturate(smoke);
                ...
                float4 refractedPos = ComputeGrabScreenPos(
                    UnityObjectToClipPos( float4( p + rd * _RefractionBackgroundSampleExtraStep , 1 ) ) );
                ...
                //UNITY_SAMPLE_TEXCUBE_LOD('cubemap', 'sample coordinate', 'map-map level')
                half3 skyColor = DecodeHDR( skyData , unity_SpecCube0_HDR );
                // This is done because the cubemap is stored HDR
                //col = skyColor;
