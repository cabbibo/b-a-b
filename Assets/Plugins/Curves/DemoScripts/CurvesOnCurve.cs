    ...
    void BuildMesh(Curve c)
    {
        ...
        for (int i = 0; i < numTubes; i++)
        {
            ...
            for (int j = 0; j < lengthSegments; j++)
            {
                ...
                float outAngle = lengthAlongTube * (spiralSpeed + spiralSpeedRandomness * randomVal1) + randomVal2 * 2 * Mathf.PI;
                float offsetAngle = lengthAlongTube * (offsetOsscilateSpeed + offsetOsscilateSpeedRandomness * randomVal2) + randomVal3 * 2 * Mathf.PI;
                ...
                for (int k = 0; k < radialSegments; k++)
                {
                    ...
                    float2 uv2 = float2(i, (float)i / (float)numTubes);
