Shader "Unlit/WaterDropVelocity"
{
    Properties
    {
   ...
   float radiusMultiplier = .3;
    ...
    /* for( int i = 0; i < 1; i++ ){
     ...
     for( int i = 0;  i < 20; i++ ){
         ...
         float timeInDrop = _Time.y * length(_Velocity) * .001 * (1+.3 * sin(float(i))) %1;
        float size = saturate(min(timeInDrop*3, 1-timeInDrop));

        float3 xOff = sin(float(i) * 100 )  * _VX;
        float3 yOff = sin(float(i) * 451.3 )* _VY;

    
         res = smoothU(res,  float2( sdSphere( pos - _MainBall.xyz + xOff + yOff+  (_MainBall.w *radiusMultiplier * .5 + timeInDrop *3)   * _Velocity , size),1   ) ,.4);
     }
...
float3 calcNormal( in float3 pos ){
	...
	return normalize(nor);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if( res.y > -.5 ){
                    col = 1;
                    col.xyz = calcNormal( ro + rd  * res.x ) * .5 + .5;
                }else{
                    discard;
                }
