...
public class WrenNetworked : RealtimeComponent<WrenNetworkedModel>
{
    ...
    public void SetHue4( float v )
    {
        if ( _model != null ) {
            ...
            wren.state.Hue4Change( v );
        }
    }

    public void SetPlayerID( uint v )
    {
        if ( _model != null ) {
            _model.playerID = v;
        } else {
            wren.state.PlayerIDDidChange( v );
        }
    }
}