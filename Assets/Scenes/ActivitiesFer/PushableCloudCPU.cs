...
public class PushableCloudCPUEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        var myScript = (PushableCloudCPU)target;
...
public class PushableCloudCPU : MonoBehaviour
{
    ...
    [Range( 0 , 1 )]
    public float playerFwdVortexForce = 1;

    public Vector3 PlayerPosition => debug || !God.wren
        ? transform.position + Vector3.forward * Mathf.Sin( Time.time * .5f ) * 20
        : God.wren.physics.transform.position;

    public Vector3 PlayerForward => debug || !God.wren ? Vector3.forward : God.wren.physics.transform.forward;
    ...
    private TinyParticle[] tinyParticles;
    private Matrix4x4[]    particleMatrices;
    private Matrix4x4[]    tinyParticlesMatrices;
    ...
    private void UpdateParticles()
    {
        ...
        for ( int i = 0; i < particleCount; i++ ) {
            ...
            float dtp = Vector3.Distance( bigParticles[i].position , pp );
            ...
            float s = bigParticleSize * bigParticles[i].size;
            ...
            if ( renderTinyParticles ) {
                for ( int j = 0; j < PARTICLES_PER_CLOUD; j++ ) {
                    ...
                    float t = (Random.value + Time.time * (1 / lifetime)) % 1;
                    pos += Random.onUnitSphere * t * 2.5f;
                    float ns = sizeCurve.Evaluate( t ) * tinyParticleSize;
