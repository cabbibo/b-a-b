...
/*
        ...
        if ( newPlucks > 0 && numPlucked > pluckedCutoff && Time.time - lastPlayTime > minPlayTime ) {
            ...
            // WrenUtils.God.audio.Play( clip , Mathf.Clamp( numPlucked * pitchMultiplier , 0 , pitchMax ) ,
            //   Mathf.Clamp( numPlucked * volumeMultiplier , 0 , volumeMax ) , 0 , 1 , WrenUtils.God.audio.defaultMixer , groupName );

            synth.NewGrain( .5f , Mathf.Clamp( numPlucked * pitchMultiplier , 0 , pitchMax ) ,
                Mathf.Clamp( numPlucked * volumeMultiplier , 0 , volumeMax ) , 0 , Random.Range( 0 , clips.Length ) ,
                .5f );
