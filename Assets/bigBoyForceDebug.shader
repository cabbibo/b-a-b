...
uniform float4x4 _BirdTransform;
...
varyings vert (uint id : SV_VertexID){
  ...
  if( base < _Count ){
    ...
    float3 bPos = mul( _BirdTransform, float4(0,0,0,1) ).xyz;

    float3 dirToBird = ( bPos - v.pos );

      o.worldPos = bPos - dirToBird * .04 + extra * _Size;// mul(_Transform, float4((v.pos) ,1));
