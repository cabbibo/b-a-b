...
[ExecuteAlways]
public class RenderLayerToSeperateTexture : MonoBehaviour
{
    ...
    RenderTexture blurredColorTexture;

    public Material depthBlurMaterial;


    public Vector2 renderSize = new Vector2(1920, 1080); // can do it whatever size we want it!
    ...
    void OnEnable()
    {
        ...
        // Create a color texture
        blurredColorTexture = new RenderTexture((int)renderSize.x, (int)renderSize.y, 0, RenderTextureFormat.ARGB32);
        blurredColorTexture.Create();

        // Create a depth texture
        blurredDepthTexture = new RenderTexture((int)renderSize.x, (int)renderSize.y, 24, RenderTextureFormat.Depth);
    ...
    void LateUpdate()
    {
        ...
        depthBlurMaterial.SetTexture("_DepthTexture", depthTexture);

        // Graphics.SetRenderTarget(blurredColorTexture.colorBuffer, blurredDepthTexture.depthBuffer);

        Graphics.Blit(depthTexture, blurredDepthTexture, depthBlurMaterial);
        //Graphics.Blit(depthTexture, blurredDepthTexture);



        debugDepthRenderer.sharedMaterial.SetTexture("_MainTex", blurredDepthTexture);
